...
#    if ismatch(r"[\,\-\:\;\}]$", word)
    ...
    if ismatch(r"[A-Za-z0-9]", word) && !(ismatch(r"[\=\>\<\/]", word)) && !(ismatch(r"\-\-", word)) && !(ismatch(r"^[0-9]+$", word)) && !(ismatch(r"^[0-9]+\.[0-9]+$", word))
...
function sort_index(reschem_array)
  ...
  for val in sort(collect(values(abdict)))
    #push!(abnew_arr, "$(getKey(abdict, val)[1])")
    for gk in getKey(abdict, val)
      push!(abnew_arr, gk)
    end
  end
  return unique(abnew_arr)
end

#generates query from given chemical to be able to look-up mysql database
function getQuery(warr, chem_dict)
  str_arr = Any[]
  cscr_dict = Dict{Any, Any}()
  for str in warr
    c1, c2, ind = split(str, r"\|")
    cscr_dict["$c1\|$c2\|$ind"] = float(chem_dict["$c1\|$c2\|$ind"])
  end
  #println(cscr_dict)
  qarr = Any[]
  if size(collect(values(cscr_dict)))[1] > 4
    for v in sort(collect(values(cscr_dict)))[1:5]
      push!(qarr, "$(getKey(cscr_dict, v)[1])")
    end
  else
    for v in sort(collect(values(cscr_dict)))
      push!(qarr, "$(getKey(cscr_dict, v)[1])")
    end
  end
  #println(qarr)
...
function lookupLevinDist(wrd, qry, rulDict)
  ...
  if size(resarr)[1] != 0
    for res in resarr
      if res != ""
        #distscr = levenshtein(wrd, res)
        distscr = compare(Levenshtein(), wrd, res)
        if distscr >= 0.7
          push!(res_arr, wrd)
        end
      end
...
function chemicalRecognition(tokind_arr)
  ...
  for tokind in tokind_arr
    ...
    #println(word)
    #if ismatch(r"[0-9]\,[0-9].*", word) #|| ismatch(r"[A-Z]+[A-Z0-9\-]+", word)
    ...
    if dict_chem == 1
      ...
      if scr == 1
        ...
        cwordlc = ASCIIString(cwordlc)
        qstr = getQuery(warr, chem_dict)
        results_ld = lookupLevinDist(cwordlc, qstr, rulDict)
        if length(results_ld) > 0
          push!(reschem_array, "$ind\$$word")
        end
      end
    end
  end
  #println("reschem \= $reschem_array")
  abnew_arr = sort_index(reschem_array)
  #println("sort \= $abnew_arr")
...
for ln in eachline(inFile)
  ln = chomp(ln)
  id, ti, ab = split(ln, r"\t")
  #ti_token_arr = unique(tokenarr(ti, lnarr, wmcarr))
  #println(ti_token_arr)
  #ti_tokind_arr = findindex(ti, ti_token_arr)

  #ti_chemical_array = chemicalRecognition(ti_tokind_arr)

  #for chem in ti_chemical_array
  #  println("$id\$T\$$chem")
  #end

  ab_token_arr = unique(tokenarr(ab, lnarr, wmcarr))
  #println(ab_token_arr)
  ab_tokind_arr = findindex(ab, ab_token_arr)
  #println(ab_tokind_arr)
  ab_chemical_array = chemicalRecognition(ab_tokind_arr)
  #println(ab_chemical_array)
  for chem in ab_chemical_array
    println("$id\$A\$$chem")
  end
end
