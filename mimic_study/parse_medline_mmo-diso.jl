...
# and retain concepts that belong to semantic group "DISORDERS"
# Sept 1st, 2017
# Run: julia parse_medline_mmo-diso.jl <path_to_input_file> <output_file>
...
function parse_mmo(seg_arr, diso_arr)
  out_arr = Any[]
          ...
          if ismatch(r"\-[0-9]+\,\[ev.*", m)
            ...
            for ev in split(m, "ev\(")
              if ismatch(r"^\-1000\,C[0-9]+", ev)
                ...
                for sty in split(evsty, "\,")
                    if in(sty, diso_arr)
                        #println("$evscr\$$evcui\$$evstr\$$sty")
                        push!(out_arr, "$evscr\$$evcui\$$evstr\$$sty")
                    end
                end
              end
            end
...
# Semantic types included withing Semantic Group: DISORDERS
diso_arr = ["acab","anab","comd","cgab","dsyn","emod","fndg","inpo","mobd","neop","patf","sosy"]
...
for txt in txt_arr
  ...
  #println(seg_arr)
  out_arr = parse_mmo(seg_arr, diso_arr)
  #println(seg_arr)
  pmid    = seg_arr[1]
