...
entries_res = mysql_execute(db, "select distinct a.string, b.ccs from medline_nhp as a inner join medline_icd9 as b on a.pmid = b.pmid where a.string like \"vit\%\";")
...
for i = 1:length(entries_res[1])
  ...
  for str in vit_strings
    ...
    a  = mysql_execute(db, "select count(distinct a.pmid) from medline_nhp as a inner join medline_icd9 as b on a.pmid = b.pmid where a.string like \"$str\" and b.ccs = \"$ccs\";")[1][1]
    #println(a)
    ...
    b  = mysql_execute(db, "select count(distinct a.pmid) from medline_nhp as a inner join medline_icd9 as b on a.pmid = b.pmid where a.string like \"$str\" and b.ccs != \"$ccs\";")[1][1]
    #println(b)
    ...
    c  = mysql_execute(db, "select count(distinct a.pmid) from medline_nhp as a inner join medline_icd9 as b on a.pmid = b.pmid where a.string not like \"$str\" and b.ccs = \"$ccs\";")[1][1]
    #println(c)
    ...
    d  = mysql_execute(db, "select count(distinct a.pmid) from medline_nhp as a inner join medline_icd9 as b on a.pmid = b.pmid where a.string not like \"$str\" and b.ccs != \"$ccs\";")[1][1]
    #println(d)

    or, lci, uci = calc_odds_ratio(a,b,c,d)
    if or > 1 && lci > 1 && a >= 5
      println("$str\$$ccs\$$or\$$lci\$$uci")
